...
def get_img_batch(secret_size,
    ...
    inds = np.random.choice(range(50000), batch_size, replace=False)
    return x_train[inds], batch_secret
...
def main():
    ...
    encoder = models.CIFAR10StegaStampEncoder(height=height, width=width)
    decoder = models.CIFAR10StegaStampDecoder(secret_size=args.secret_size, height=height, width=width)
    ...
    total_steps = 50000 // args.batch_size + 1
    global_step = 0

    while global_step < args.num_steps:
        for _ in range(min(total_steps, args.num_steps - global_step)):
            no_im_loss = global_step < args.no_im_loss_steps
            images, secrets = get_img_batch(secret_size=args.secret_size,
